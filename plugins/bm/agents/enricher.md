...
3. **Tags**: pick from `tags.yaml` `name` entries first. Use each entry's `description` to judge fit; use `aliases` to map synonyms (page is about "ML" and `ml-research` has `aliases: [machine-learning, ml]` → pick `ml-research`, not invent `ml`). Use `proposed_tags` for any concept that is topically significant for this page and not already covered by an existing tag's `name`, `description`, or `aliases` — for example, a specific programming language, framework, protocol, or domain that appears in no existing entry. Don't be stingy: if a clear, useful concept is missing from the vocabulary, propose it. Still prefer existing tags when they fit even loosely. If the vocab is empty (bootstrap), `tags: []` is correct and strong-signal new tags go in `proposed_tags`.
