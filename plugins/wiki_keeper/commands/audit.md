---
description: Health-check the vault — contradictions, stale claims, broken refs, drift
argument-hint: "[scope: full | wiki | concepts | claims | entities | recent | sources | lists | external | schema]"
---
...
2. Determine scope from `$ARGUMENTS`:
   - `full` (default): **all scopes below — wiki + sources + lists + external + schema.** Run each in turn and merge findings.
   - `wiki`: all wiki pages only.
   - `concepts` / `claims` / `entities`: that wiki subtree only.
...
3. For wiki scopes (`wiki`, `concepts`, `claims`, `entities`, `recent`): spawn the `wiki-auditor` subagent. It runs in clean context, reads pages, returns findings.
   ...
   For `full`: run every scope. Prefer parallel agent invocations where possible. Merge findings into a single severity-grouped table.
...
5. Ask the user which findings to act on. Apply corrections one at a time. Do not bundle.

## Wiki auditor categories (wiki / concepts / claims / entities / recent)
...
- Use the audit to refactor the vault structurally. Structural changes go through a separate decision.
